...
test('streams a live CAS turn through the packaged AHP server process', {
  skip: process.env.NATS_URL && (process.env.CODEX_APP_SERVER_URL || process.env.CODEX_APP_SERVER_SOCKET)
    ? false
    : 'set NATS_URL and CODEX_APP_SERVER_URL or CODEX_APP_SERVER_SOCKET to run live server-process validation',
  timeout: 120_000,
}, async () => {
  assert.ok(process.env.NATS_URL);
  const storageDirectory = mkdtempSync(join(tmpdir(), 'ahp-server-process-'));
  const namespace = `ahp.live.process.${Date.now()}`;
  ...
  try {
    ...
    await client.initialize({
      clientId: 'live-process-client',
      protocolVersions: ['0.3.0'],
      initialSubscriptions: ['ahp-root://'],
    });

    sessionUri = `ahp-session:/live-process-${Date.now()}`;
    ...
    while (Date.now() < deadline) {
      ...
      if (next.done || next.value.type !== 'action') {
        continue;
      }
