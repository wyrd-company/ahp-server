...
updatedAt: '2026-06-13T21:26:46.481Z'
role: summary
alwaysLoad: false
...
Validation passed: `npm run verify` in `ahp-grpc`, `ahp-nats`, `ahp-server`, `a2a-nats`, and `a2a-ahp`. Live NATS validation also passed against a temporary Docker NATS broker for `ahp-server` `test/nats-live.test.ts` and `a2a-nats` `test/nats-docker.test.ts`, including JetStream and KV tests.## Server Transport Re-Export RemovedOn 2026-06-13, `ahp-server` commit `21566ef` removed the `./nats` and `./grpc` package subpath exports and deleted the `src/nats/index.ts` and `src/grpc/index.ts` re-export shims. Consumers now import transport symbols directly from `@wyrd-company/ahp-nats` and `@wyrd-company/ahp-grpc` when composing an AHP host. The packaged server process still composes those transport packages internally, but the core server package no longer presents them as part of its public library barrel.Validation passed after the cleanup: `npm run verify` in `ahp-server`, `ahp-nats`, and `ahp-grpc`; `npm pack --dry-run` in `ahp-server` confirmed the package no longer includes `dist/src/nats` or `dist/src/grpc`. Live NATS tests in `ahp-server` were skipped because `NATS_URL` was not set.## Packaged Host Removed From Server CoreOn 2026-06-13, `ahp-server` commit `879e82b` removed the built-in CLI/process host. The package no longer exposes a `bin`, no longer exports `./process`, and no longer has runtime dependencies on `@nats-io/transport-node`, `@wyrd-company/ahp-nats`, or `@wyrd-company/ahp-grpc`. Host applications are expected to compose `AhpServer`, storage, transports, and providers explicitly.Deleted process-owned tests and scripts included the gRPC process test, process config test, live NATS process resource test, NATS server integration tests owned by the server package, and the `task live:resources` script. A new in-memory multi-client test keeps coverage for concurrent `AhpServer.accept(...)` connections without coupling the core package to transport implementations. Validation passed: `npm run verify` and `npm pack --dry-run` in `ahp-server`.## Cross-Transport Session Join Use CaseOn 2026-06-13, Bob confirmed the intended orchestration/viewer flow: an orchestrating agent sends A2A client -> A2A server -> active AHP client -> in-process transport -> `AhpServer`; later a human/user view connects as another AHP client over NATS and subscribes to the same AHP session URI. This is acceptable without additional permission policy for now.`ahp-server` commit `c709e14` added a regression test proving that a session created through one accepted transport can be subscribed to from another accepted transport. The joining client receives the existing session snapshot and later session actions. Validation passed with `npm run verify`.
