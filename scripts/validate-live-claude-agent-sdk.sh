...
NATS_CONTAINER=${NATS_CONTAINER:-ahp-server-claude-nats-validation}
NATS_IMAGE=${NATS_IMAGE:-nats:2.10-alpine}
CLAUDE_AGENT_SDK_LIVE_TURN_PROMPT=${CLAUDE_AGENT_SDK_LIVE_TURN_PROMPT:-Reply with exactly: pong}
...
export CLAUDE_AGENT_SDK_LIVE_TURN_PROMPT

cd "$ROOT_DIR"
npm run build
node --test --import tsx test/claude-agent-sdk-live.test.ts test/claude-agent-sdk-process-live.test.ts
