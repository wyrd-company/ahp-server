...
NATS_CONTAINER=${NATS_CONTAINER:-ahp-server-process-nats-validation}
...
if [[ -z "${CODEX_APP_SERVER_URL:-}" && -z "${CODEX_APP_SERVER_SOCKET:-}" ]]; then
  socket_dir=${CODEX_APP_SERVER_SOCKET_DIR:-/tmp/ahp-cas-process}
  rm -rf "$socket_dir"
  mkdir -p "$socket_dir"
  chmod 700 "$socket_dir"
  export CODEX_APP_SERVER_SOCKET="$socket_dir/app-server-control.sock"
  codex app-server --listen "unix://$CODEX_APP_SERVER_SOCKET" >/tmp/ahp-server-cas-process-live.log 2>&1 &
  cas_pid=$!
  started_cas=1
  for _ in $(seq 1 100); do
    ...
    if ! kill -0 "$cas_pid" >/dev/null 2>&1; then
      cat /tmp/ahp-server-cas-process-live.log >&2
      exit 1
    fi
    sleep 0.1
  done
...
cd "$ROOT_DIR"
npm run build
node --test --import tsx test/server-process-live.test.ts
