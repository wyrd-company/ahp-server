...
NATS_CONTAINER=${NATS_CONTAINER:-ahp-server-pi-nats-validation}
NATS_IMAGE=${NATS_IMAGE:-nats:2.10-alpine}
PI_AGENT_LIVE_TURN_PROMPT=${PI_AGENT_LIVE_TURN_PROMPT:-Reply with exactly: pong}
...
export PI_AGENT_LIVE_TURN_PROMPT

cd "$ROOT_DIR"
npm run build
node --test --import tsx test/pi-agent-live.test.ts test/pi-agent-process-live.test.ts
